...
> 所有扫描模式（Light/Deep）共享相同的 remediation 输出格式。
...
security-scan-output/{batch}/
  agents/
    recon-lite.json          # Deep 模式
    recon-deep.json           # Deep 模式
    light-scan.json         # Light 模式专用
    quick-scan.json           # Deep 模式
    deep-scan.json            # Deep 模式
    verification.json         # Deep 模式
    remediation.json          # 编排器内联产出（所有模式）
  stage1-context.json
  pattern-scan-results.json   # 共享扫描结果（由 quick-scan 或 light-scan 写入）
  merged-scan.json            # merge_findings.py merge-scan 输出（Deep 模式）
