...
def extract_findings_from_pattern_scan(data):
    ...
    for f in data.get('cveFindings', []):
        ...
        risk_detail = f.get('description') or f.get('RiskDetail') or f.get('RiskDescription') or ''
...
def merge_stage3(batch_dir, prefix=''):
    ...
    for slug, group_findings in sorted(groups.items()):
        ...
        for f in group_findings:
            sev = normalize_severity(f.get('severity') or f.get('RiskLevel') or f.get('riskLevel') or 'medium')
            if sev == 'critical':
                critical += 1
            elif sev == 'high':
                high += 1
            ...
            risk_item = {
                ...
                "RiskDetail": f.get('RiskDetail') or f.get('riskDetail') or f.get('RiskDescription') or f.get('riskDescription') or '',
                "Suggestions": f.get('Suggestions') or f.get('suggestions') or f.get('fixDescription') or '',
